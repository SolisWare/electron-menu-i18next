...
Please do not include secrets, credentials, or personal information that are
not required to reproduce the issue.
...
If an issue originates in Electron, i18next, or another dependency, report it
to that project's security team when the source is clear. If the source is
uncertain or the issue involves this package's integration with a dependency,
report it to SolisWare first.
